...
$default_sort_order_type   = $_CFG['sort_order_type'] == '0' ? 'sales_volume' : ($_CFG['sort_order_type'] == '1' ? 'sales_volume' : 'last_update');
...
function category_get_goods($children, $brand, $min, $max, $ext, $size, $page, $sort, $order)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res))
    {
		...
		{
			$nowprice = $promote_price; //现价	
		}
		else
		{
			$nowprice = $row['shop_price']; //现价	
		}
		$arr[$row['goods_id']]['jiesheng'] = $price-$nowprice; //节省金额 
		if($nowprice > 0)
		{
			 $arr[$row['goods_id']]['zhekou'] = round(10 / ($price / $nowprice), 1); 
		}
		else 
		{ 
			$arr[$row['goods_id']]['zhekou'] = 0; 
		}
