...
?> 
	     ...
	     <input type="text" name="price_max" id="price_max" value="<?php echo $this->_var['price_max']; ?>" class="price_68ecshop" onkeyup="this.value=this.value.replace(/\D/g,'')" onafterpaste="this.value=this.value.replace(/\D/g,'')" autocomplete="off">
          ...
          <?php $_from = $this->_var['filter_attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('kattr', 'filter_attr_0_43600000_1527131520');$this->_foreach['filter_attr_list'] = array('total' => count($_from), 'iteration' => 0);
if ($this->_foreach['filter_attr_list']['total'] > 0):
    foreach ($_from AS $this->_var['kattr'] => $this->_var['filter_attr_0_43600000_1527131520']):
...
?>
          <dl  name="attr_group_dl" id="attr_group_dl_<?php echo $this->_foreach['filter_attr_list']['iteration']; ?>" >
            <dt><?php echo htmlspecialchars($this->_var['filter_attr_0_43600000_1527131520']['filter_attr_name']); ?>：</dt>
            <dd class="moredd">
            ...
            <dd>
              <ul id="attr_abox_<?php echo $this->_foreach['filter_attr_list']['iteration']; ?>">
                <?php $_from = $this->_var['filter_attr_0_43600000_1527131520']['attr_list']; if (!is_array($_from) && !is_object($_from)) { settype($_from, 'array'); }; $this->push_vars('', 'attr');$this->_foreach['attr_list_68ecshop'] = array('total' => count($_from), 'iteration' => 0);
...
?> 
                ...
                <li> <?php if ($this->_var['filter_attr_0_43600000_1527131520']['filter_attr_name'] == '颜色'): ?> <span class="color_ecshop68" style="position:relative;">
