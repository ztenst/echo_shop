...
function shipping_list()
{
    $sql = 'SELECT shipping_id, shipping_name ' .
            'FROM ' . $GLOBALS['ecs']->table('touch_shipping') .
            ' WHERE enabled = 1';
...
function available_shipping_list($region_id_list)
{
    $sql = 'SELECT s.shipping_id, s.shipping_code, s.shipping_name, ' .
            ...
            'FROM ' . $GLOBALS['ecs']->table('touch_shipping') . ' AS s, ' .
                $GLOBALS['ecs']->table('touch_shipping_area') . ' AS a, ' .
...
function shipping_fee($shipping_code, $shipping_config, $goods_weight, $goods_amount, $goods_number='')
{
    ...
    $filename = ROOT_PATH . 'include/modules/shipping/' . $shipping_code . '.php';
...
function shipping_insure_fee($shipping_code, $goods_amount, $insure)
{
    ...
    {
        $path = ROOT_PATH . 'include/modules/shipping/' . $shipping_code . '.php';
...
function &get_shipping_object($shipping_id)
{
    ...
    $file_path = ROOT_PATH.'include/modules/shipping/' . $shipping['shipping_code'] . '.php';
...
{
    $sql = "SELECT pay_id FROM " . $GLOBALS['ecs']->table('touch_payment');
    if ($is_cod)
    {
        $sql .= " WHERE is_cod = 1";
    }
