...
{
    /* 获得配置信息 */
    $sql = 'SELECT configure FROM ' . $GLOBALS['ecs']->table('touch_shipping_area') . " WHERE shipping_area_id = '$area_id'";
...
function build_uri($app, $params, $append = '', $page = 0, $keywords = '', $size = 0)
{
    ...
    switch ($app)
    {
		...
		$parent_ranks =  $GLOBALS['db']->getOne("SELECT special_rank FROM " . $GLOBALS['ecs']->table('user_rank')."where rank_id=".$rank_id);
		if($parent_ranks==1){
			if(!empty($userid)){
