...
elseif ($_REQUEST['act'] == 'info')
{
    ...
    {
        /* 查询广告的名称 */
         $ad_name = $db->getOne("SELECT ad_name FROM " .$ecs->table('touch_ad'). " WHERE ad_id='$order[from_ad]'");
    ...
    while ($row = $db->fetchRow($res))
    {
        ...
        {
            /* 取得语言项 */
            $filename = ROOT_PATH . 'plugins/' . $row['extension_code'] . '/lang/common_' . $_CFG['lang'] . '.php';
    ...
    elseif (isset($_GET['shipping_print']))
    {
        ...
        $smarty->assign('shop_address', $_CFG['shop_address']);
        $smarty->assign('service_phone',$_CFG['service_phone']);
        $shipping = $db->getRow("SELECT * FROM " . $ecs->table("touch_shipping") . " WHERE shipping_id = " . $order['shipping_id']);
        ...
        {
            $shipping_code = $db->getOne("SELECT shipping_code FROM " . $ecs->table('touch_shipping') . " WHERE shipping_id=" . $order['shipping_id']);
            if ($shipping_code)
            {
...
elseif ($_REQUEST['act'] == 'templates')
{
    ...
    include_once(ROOT_PATH . 'include/fckeditor/fckeditor.php');
