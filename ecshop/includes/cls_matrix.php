...
class matrix
{
    ...
    function getOrderStruct($order_sn,$fields='*',$is_create=FALSE){
        ...
        // 打ERP 有货号的贺卡需作为普通商品 商品总金额要加上贺卡金额
        $paramss['total_goods_fee'] = $this->format_number($order_info['goods_amount'] + $order_info['card_fee']);//商品总额
        ...
        $paramss['discount_fee'] = $this->format_number($order_info['discount']);//折扣优惠金额
        // 包装费用
        if ($order_info['pack_fee'] > 0) {
            $service_order_objects = array(
                'service_order' => array(
                    array(
                        'item_oid'       => '',
                        'refund_id'      => '',
                        'sale_price'     => (float)$order_info['pack_fee'],
                        'oid'            => '',
                        'tmser_spu_code' => '',
                        'num'            => 1, // 先默认为1
                        'total_fee'      => (float)$order_info['pack_fee'],
                        'title'          => 'pack_fee',
                        'service_id'     => '',
                        'type_alias'     => '包装费用',
                        'type'           => 'pack_fee',
            ...
            $paramss['service_order_objects'] = json_encode($service_order_objects);
            $paramss['service_orders'] = json_encode($service_order_objects); // ERP单拉订单的时候用的是service_orders
        }
        ...
        $paramss['buyer_phone'] = $memberinfo['office_phone']; // 办公电话
        // $paramss['buyer_phone'] = $memberinfo['home_phone']; // 家庭电话
        $paramss['buyer_email'] = $memberinfo['email']; // 电子邮件地址
