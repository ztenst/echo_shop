...
$payment_lang = ROOT_PATH . 'languages/' .$GLOBALS['_CFG']['lang']. '/payment/tiangongcp.php';
...
if (isset($set_modules) && $set_modules == TRUE)
{
    ...
    /* 描述对应的语言项 */
    $modules[$i]['desc']    = 'tiangongcp_desc';
    ...
    $modules[$i]['config']  = array(
        array('name' => 'tiangongcp_client_id',           'type' => 'text',   'value' => ''),
        array('name' => 'tiangongcp_client_secret',           'type' => 'text',   'value' => ''),
    );
...
class tiangongcp
{
    ...
    function get_code($order, $payment)
    {
       ...
       // $param['return_url'] = 'http://www.qq.com';
        ...
        $param['client_id'] = $payment['tiangongcp_client_id'];
...
//        error_log(print_r($param,1)."\n~~~~",3,"/Users/roshan/www/ecshop/admin/ecshop.log");
        ...
        $def_url  = '<div style="text-align:center"><form name="tiangongcp" accept-charset="UTF-8" style="text-align:center;" method="post" action="https://api.teegon.com/charge/pay" target="_blank">';
       ...
       // echo "<pre/>";
        ...
        if ($_GET['is_success'] == 'true')
        {
            ...
            if(!empty($_POST))
            {
                ...
                $tgsign = md5($tgreturn.$payment['tiangongcp_client_secret']);
...
//tiangongcp 加密算法
    public function sign($para_temp,$payment){
        ...
        $prestr = $payment['tiangongcp_client_secret'] .$prestr . $payment['tiangongcp_client_secret'];
